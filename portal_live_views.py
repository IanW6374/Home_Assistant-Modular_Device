...
def update_upload_script():
    return (
        ...
        'function sendUniversal(){startPolling();return new Promise(function(resolve,reject){var request=new XMLHttpRequest();'
        'request.open("POST","/universal-upload",true);request.setRequestHeader("Content-Type","application/octet-stream");'
        'request.setRequestHeader("X-CSRF-Token",csrfToken);request.setRequestHeader("X-Update-ID",id);'
        'request.upload.onprogress=function(e){if(e.lengthComputable)label.textContent="Uploading "+Math.round(e.loaded*100/e.total)+"%";};'
        'request.upload.onload=function(){label.textContent="Writing core firmware 0%";previous("Completed: universal upload");};'
        'request.onload=function(){if(request.status===401){location.replace("/login");reject(new Error("Session expired"));return;}'
        'if(request.status===202||(request.status>=200&&request.status<300)){resolve();return;}'
        'reject(new Error(request.responseText||"Universal update failed"));};'
        'request.onerror=function(){reject(new Error("Universal upload connection failed"));};request.send(f);});}'
        ...
        '(firmware?"firmware":"application");if(universal)return sendUniversal().then(function(){return null;});'
        'return jsonPost("/resumable-upload-begin",{id:id,kind:kind,total_bytes:f.size,sha256:hex});'
        '}).then(function(s){if(!s)return;startPolling();return sendChunk(Number(s.received_bytes||0));}).catch(function(err){finished=true;box.classList.add("failed");'
