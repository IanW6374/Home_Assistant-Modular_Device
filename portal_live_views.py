...
def update_upload_script():
    return (
        ...
        'function uploadChunk(url,blob,base,total,prefix){return new Promise(function(resolve,reject){var request=new XMLHttpRequest();'
        'request.open("POST",url,true);request.withCredentials=true;request.setRequestHeader("Content-Type","application/octet-stream");'
        'request.setRequestHeader("X-CSRF-Token",csrfToken);request.upload.onprogress=function(event){if(!event.lengthComputable)return;'
        'var percent=Math.min(99,Math.round((base+event.loaded)*100/total));label.textContent=prefix+percent+"%";};'
        ...
        'request.onerror=function(){reject(new Error("Chunk upload failed"));};request.send(blob);});}'
        'function sendChunk(offset){if(offset>=f.size){label.textContent="Checking uploaded "+(firmware?"core firmware":"application")+" bytes";'
        ...
        'url,f.slice(offset,end),offset,f.size,"Uploading ").then(function(s){'
        ...
        '{label.textContent="Verifying signed "+name+" "+(s.percent||0)+"%";}else if(s.phase==="writing"||s.phase==="firmware_writing")'
        '{label.textContent="Writing "+name+" "+(s.percent||0)+"%";}else if(s.phase==="compacting")'
        '{label.textContent="Staging "+name+" "+(s.percent||0)+"%";}if(s.phase==="complete")'
        ...
        'label.textContent="Preparing upload…";f.arrayBuffer().then(function(data){return crypto.subtle.digest("SHA-256",data);}).then(function(hash){var hex=Array.from(new Uint8Array(hash)).map(function(b){'
