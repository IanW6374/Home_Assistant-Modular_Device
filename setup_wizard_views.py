...
def _page(csrf, message='', invalid_fields=(), values=None):
    ...
    return (
        ...
        '<label class="field">Network name (SSID)<span class="wifi-ssid-control">'
        '<select id="wifi-ssid-select" name="wifi_ssid" required data-current="' + _escape(wifi_ssid) + '">'
        '<option value="' + _escape(wifi_ssid) + '" selected>' +
        _escape(wifi_ssid or 'Select a detected network…') + '</option>'
        '<option value="__manual__">Enter network name manually…</option></select>'
        '<span id="wifi-ssid-manual" class="wifi-ssid-manual" hidden>'
        '<input id="wifi-ssid-input" disabled required maxlength="32" placeholder="Enter network name">'
        '<button id="wifi-ssid-detected" class="secondary compact" type="button">Detected networks</button>'
        '</span></span></label>'
        ...
        'placeholder="iot-md-001.local" pattern="[A-Za-z0-9-]+\\.local" value="' +
        field_value('certificate_hostname', 'iot-md-001.local') + '"></label></div>'
        ...
        'wifiOption("__manual__","Enter network name manually…"));wifiSelect.value=current||"";setWifiStatus('
...
def _certificate_page(csrf, hostname='', message='', ready=False, error=False):
    ...
    return (
        ...
        '<option value="self_signed">Self-signed device certificate</option>'
        '<option value="iot_ca_auto">Automatic IoT CA enrollment</option>'
        '<option value="iot_ca_file">IoT CA enrollment authorization (.iotenroll)</option>'
        ...
        '<div class="section-title"><h2>Self-signed device certificate</h2></div>'
        ...
        '>Continue with self-signed device certificate</button></form></section>'
        ...
        '<div class="section-title"><h2>IoT CA enrollment authorization (.iotenroll)</h2></div>'
        ...
        '<input type="hidden" name="csrf" value="' + _escape(csrf) + '">'
        '<label class="field">IoT CA enrollment authorization'
