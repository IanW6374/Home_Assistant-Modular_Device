...
class RemoteLoggingTests(unittest.TestCase):
    def test_rfc5424_message_uses_local0_and_expected_severity(self):
        payload = rfc5424_message(
            '2026-08-22T12:00:00', 'controller', 'IoT-MD', 'Started', 'ERROR'
        ).decode()

        self.assertTrue(payload.startswith('<131>1 2026-08-22T12:00:00Z controller IoT-MD'))
    ...
    def test_system_and_audit_forwarding_are_independent(self):
        ...
        self.assertIn(b'IoT-MD-Audit', audit_only.queue[0])
        ...
        self.assertNotIn(b'IoT-MD-Audit', system_only.queue[0])
