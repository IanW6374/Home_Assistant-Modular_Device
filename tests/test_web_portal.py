...
class WebPortalTests(unittest.TestCase):
    ...
    def test_render_page_has_auto_refresh_and_scrollable_logs(self):
        ...
        self.assertIn('request.open("POST","/universal-upload",true)', updates)
        self.assertIn('request.setRequestHeader("X-Update-ID",id)', updates)
        self.assertIn('if(universal)return sendUniversal()', updates)
